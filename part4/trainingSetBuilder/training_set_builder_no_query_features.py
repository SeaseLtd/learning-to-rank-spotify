...
def write_training_test_set(data_frame, output_dir, test_set_size=10000, query_id_sample_threshold=25):
    ...
    interactions_train, interactions_test = training_test_set_split(
       interactions_data_frame, query_id_sample_threshold, percentage, output_dir)
...
def training_test_set_split(interactions_data_frame, query_id_sample_threshold, percentage, output_dir):
    ...
    utils.write_set(under_sampled_only_train, output_dir, 'under_sampled_only_train')
...
def training_set_builder_remove_query_level_features(output_dir, dataset_name, relevance_label_number, test_set_size, query_id_sample_threshold, remove_query_level_features):
    # Load the dataset obtained after the preprocessing part
    new_dataset_store = pd.HDFStore(output_dir+'/'+dataset_name+'.h5', 'r')
    new_dataset = new_dataset_store[dataset_name]
    new_dataset_store.close()
    ...
    new_dataset['Region'] = np.nan
    new_dataset['Month'] = np.nan
    new_dataset['Day'] = np.nan
    new_dataset['Weekday'] = np.nan
